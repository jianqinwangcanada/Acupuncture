...
namespace Acupuncture.CommonFunction.RoleSvc
{
    public class RoleSvc:IRoleSvc
    {
        ...
        public RoleSvc(RoleManager<IdentityRole> roleManager, UserManager<ApplicationUser> userManager, IWebHostEnvironment env, ApplicationDbContext db)
        {
        ...
        public async Task<bool> UpdateRoleAsync(IFormCollection formData)
        {
            ...
            try
            {
                ...
                if (roleToUpdate != null)
                {
                    ...
                    //Remove additional permissions from new permission list
                    ...
                    // roleToUpdate.Permissions.Select(item=>
                    //    applicationRole.Permissions.FirstOrDefault(x => x.Id == item.Id)


                    //);
                 
                    roleToUpdate.Permissions.Select(c => {
                        c.Add = applicationRole.Permissions.FirstOrDefault(x => x.Id == c.Id).Add;
                        c.Delete = applicationRole.Permissions.FirstOrDefault(x => x.Id == c.Id).Delete;
                        c.Update = applicationRole.Permissions.FirstOrDefault(x => x.Id == c.Id).Update;
                        c.Read = applicationRole.Permissions.FirstOrDefault(x => x.Id == c.Id).Read;
                        return c;
                    }).ToList();
                    // var items = roleToUpdate.Permissions.Cast<RolePermission>().ToList();
                    // roleToUpdate.Permissions =
                    //     items.Select(permission => applicationRole.Permissions.FirstOrDefault(x => x.ApplicationRoleId == permission.ApplicationRoleId)).ToList();
                    //// roleToUpdate.Permissions = applicationRole.Permissions;
                    /*    miss one line*/
                    //That means the old role's permissions cann't be changed
        ...
        public async Task<bool> DeleteRolePermissionTypesAsync(int TypeId)
        {
            var resultError = false;

            var TypeToDelete = await _db.PermissionTypes.FindAsync(TypeId);

            var PermissionsToDelete = await _db.RolePermissions.FirstOrDefaultAsync(x=>x.Type==TypeToDelete.Type);

            
            if (TypeToDelete != null)
            {
                _db.PermissionTypes.Remove(TypeToDelete);
                _db.RolePermissions.Remove(PermissionsToDelete);
                await _db.SaveChangesAsync();
            }
            else
            {
        ...
        public async Task<bool> UpdatePermissionTypeAsync(PermissionType permissionType)
        {
            var resultError = false;
            ...
            try
            {
                ...
                var permissionTypeToUpdate = _db.PermissionTypes.FirstOrDefault(o => o.Id == permissionType.Id);

                if (permissionTypeToUpdate != null)
                {
                    permissionTypeToUpdate.Type = permissionType.Type;

                    _db.Entry(permissionTypeToUpdate).State = EntityState.Modified;
                    await _db.SaveChangesAsync();
                }
