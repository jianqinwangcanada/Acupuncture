...
namespace Acupuncture.CommonFunction.CountryFunction
{
    public class CountrySvc:ICountrySvc
    {
        public async Task<List<CountryModel>> GetCountries()
        {
            ...
            try
            {
                countryModels = await CreateList();
            }
            catch (Exception ex)
            {
                Log.Error("Error while Get Country Models in Country Svc {Error} {StackTrace} {InnerException} {Source}",
                   ex.Message, ex.StackTrace, ex.InnerException, ex.Source);
            }
            return countryModels;
        }
        private Task<List<CountryModel>> CreateList()
        {
            return Task.Run(() => {
